...
var runSim = function compute() {
  ...
  //values
  const revenues = parseInt(document.getElementById('revenus').value); const age = parseInt(document.getElementById('age').value); const monthlyDeposit = parseInt(document.getElementById('mensual').value); const firstDeposit = parseInt(document.getElementById('initial').value); const annualDeposit = Math.round(12 * monthlyDeposit); const rate = 0.05; const t = 63 - age; var myData = []; var dataYears = []; var dataDeposits = []; var dataInterests = []; var dataFv = [];

  //CalcFV
  let generatedInterests = 0; let annualAmount = 0; let md = 0; let fv = 0; let deposit = 0; for (let i = 1; i <= t; i++) { if (i == 1) deposit = firstDeposit + annualDeposit; else deposit = fv + annualDeposit; let interests = deposit * rate; generatedInterests += interests; fv = deposit + interests; dataFv.push(Math.round(fv)); dataYears.push(2020 + i); dataDeposits.push(Math.round(fv - generatedInterests)); dataInterests.push(Math.round(generatedInterests)); myData.push([i, Math.round(fv - generatedInterests), Math.round(generatedInterests)]) }
  ...
  //PerteRevenu
  if (urlPro == "employed") { $("#pSituation").text('salarié'); $('#pLoss').text('36,8%'); $('#pRevenu').text(currency(Math.round(revenues * 0.632))); $('#pRevenuC').text(currency(fvMensual + Math.round(revenues * 0.632))) } else if (urlPro == "tns") { $("#pSituation").text('indépendant'); $('#pLoss').text('50%'); $('#pRevenu').text(currency(Math.round(revenues * 0.5))); $('#pRevenuC').text(currency(fvMensual + Math.round(revenues * 0.5))) } else { $('#pRevenu').text(currency(Math.round(revenues * 0.5))); $('#pRevenuC').text(currency(fvMensual + Math.round(revenues * 0.5))) }; $('#fvMpension').text("+" + fvMensual + "€");
